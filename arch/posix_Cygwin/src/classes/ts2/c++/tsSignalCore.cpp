...
// Cygwin's <signal.h> (3.6.x) does not expose SA_NOCLDWAIT. It is only
// meaningful for SIGCHLD, and the teardown path below merely resets the
// handler to SIG_DFL, so a 0 fallback is behaviourally safe. Windows-port design memo.
#ifndef SA_NOCLDWAIT
